...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return await this.reply(`Команда \`${this.command.name}\` має виконуватись на серверному каналі.`);
		}
		if(!this.command.hasPermission(this)) {
			this.client.emit('commandBlocked', this, 'permission');
			return await this.reply(`У вас нема прав використовувати команду \`${this.command.name}\`.`);
		}
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return await this.reply(
				`Ви не можете використати \`${this.command.name}\` команду знову в найближчі ${remaining.toFixed(1)} секунд.`
			);
		}
		...
		if(!args && this.command.args) {
			...
			if(!args) return await this.reply('Команда відмінена.');
		...
		try {
			this.client.emit('debug', `Виконую команду ${this.command.groupID}:${this.command.memberName}.`);
			...
			if(err instanceof FriendlyError) {
				...
				return await this.reply(stripIndents`
					Виникла помилка під час виконання команди: \`${err.name}: ${err.message}\`
					Ви не повинні отримувати такі повідомлення.
					Зв'яжіться з ${ownerList}${invite ? ` на цьому сервері: ${invite}` : '.'}
				`);
			}
		}
	}
